...
import { useCart } from '../../context/cart.context'; // llamamos al contexto del carrito
...
const CollectionPage = () => {
    ...
    const { addToCart } = useCart();// llamamos a la funcion del contexto addToCart
    // la funcion addToCartHandler conecta con el servicio y con el contexto y luego se llama dentro del return 
    const addToCartHandler = async (productId, title, price, image) => {
        try {
            // Llamar al servicio para agregar el producto al carrito
            await ApiService.addToCart(productId, title, price, image);// esto es del service
            addToCart({ id: productId, title, price, image }); // esto es del contexto
